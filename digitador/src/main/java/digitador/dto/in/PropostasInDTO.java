...
public class PropostasInDTO {
    ...
    private LotesDomain loteDomain; // Lote ao qual a proposta pertence
    private EnumStatusPropostas enumStatusPropostas;
    private String linkFormalizacao;

    public PropostasInDTO(PropostasDomain proposta) {
        ...
        this.loteDomain = proposta.getLoteDomain(); // Referência ao lote
        this.enumStatusPropostas = proposta.getEnumStatusPropostas();
        this.linkFormalizacao = proposta.getLinkFormalizacao();
    }

    public static List<PropostasInDTO> convert(List<PropostasDomain> propostas) {
        return propostas.stream().map(PropostasInDTO::new).collect(Collectors.toList());
    }
}
